...
public class Scores {
	...
	public Scores(Main pl) {
		...
		if (sMethod.equalsIgnoreCase("mysql")) {
			try {
				initDatabase("mysql", ip, username, password, database, tablePrefix, useSSL, allowPublicKeyRetrieval,
						minCount, maxCount, tx_isolation);
			} catch (Exception e) {
		...
		if (method == null) {
			plugin.getLogger().severe(
					"Unable to find database method! Check storage.yml as you most likely put in an invalid storage method.");
		}
	}
	...
	private void checkStorageConfig() {
		...
		v.put("mysql.tablePrefix", "naturalparkour_");
	...
	private void initDatabase(String method, String ip, String username, String password, String database,
			String tablePrefix, boolean useSSL, boolean allowPublicKeyRetrieval, int minConnections, int maxConnections,
			String tx_isolation) throws SQLException {
		if (method.equals("mysql")) {
			url = "jdbc:mysql://" + ip + "/" + database + "?useSSL=" + useSSL + "&allowPublicKeyRetrieval="
					+ allowPublicKeyRetrieval + "&characterEncoding=utf8";
	...
	private void createTables() throws SQLException {
		...
		conn.createStatement().executeUpdate(
				"create table if not exists " + tablePrefix + "scores " +
						...
						+ ", area TINYTEXT, player VARCHAR(36), score INT, time INT)");
		closeConn(conn);
	}
	...
	private void convertFromOldSQL(String oldTable) throws SQLException {
		...
		if (teCheck.next()) {
			...
			try {
				...
				plugin.getLogger().severe(
						"Unable to remove old data from storage config. Conversion may happen again on next restart!");
				e.printStackTrace();
			}
			return;
		}
	...
	public void insertJsonData(UUID uuid, String name, String raw, int time, String mat, int gamesPlayed)
			throws SQLException {
		Connection conn = getConnection();
		if (mat == null) {
			mat = "NULL";
	...
	public int getHighScore(UUID uuid, String area) {
		...
		try {
			...
			ResultSet rs = conn.createStatement().executeQuery(
					"select score from " + tablePrefix + "scores where player='" + uuid.toString() + "' and area='"
							+ area + "'");
			if (!rs.next()) {
				closeConn(conn, rs);
				return 0;
	...
	public int getTime(UUID uuid, String area) {
		...
		try {
			...
			ResultSet p = conn.createStatement().executeQuery(
					"select time from " + tablePrefix + "scores where player='" + uuid.toString() + "' and area='"
							+ area + "'");
			if (!p.isBeforeFirst()) {
				return 0;
			}
			...
			Bukkit.getLogger().severe("[NaturalParkour] An error occurred when attempting to get a players time:");
			e.printStackTrace();
			return -1;
		}
	}
		...
		Runnable r = () -> {
			...
			try {
				...
				ResultSet r1 = conn.createStatement().executeQuery(
						"select id from " + tablePrefix + "scores where player='" + uuid.toString() + "' and area='"
								+ ar + "'");
				if (!r1.next()) {
					if (!(score == 0 && time == 0)) {
						conn.createStatement().executeUpdate("insert into " + tablePrefix + "scores " +
					...
					if (score == 0 && time == 0) {
						conn.createStatement().executeUpdate(
								"delete from `" + tablePrefix + "scores` where player='" + uuid.toString()
										+ "' and area='" + ar + "'");
					} else {
						conn.createStatement().executeUpdate(
				...
				Bukkit.getLogger().severe("[NaturalParkour] Unable to set score for a player:");
				e.printStackTrace();
			}
	...
	public String getMaterial(UUID uuid) {
		try {
			...
			ResultSet r = conn.createStatement()
					.executeQuery("select material from " + tablePrefix + "players where id='" + uuid.toString() + "'");
			if (!r.next()) {
				closeConn(conn, r);
				return "RANDOM";
			}
	...
	public String getName(UUID uuid) {
		try {
			Connection conn = getConnection();
			ResultSet r = conn.createStatement()
					.executeQuery("select name from " + tablePrefix + "players where id='" + uuid.toString() + "'");
			if (!r.next()) {
				return null;
			}
			...
			Bukkit.getLogger().severe("[NaturalParkour] An error occurred when attempting to get a players name:");
			e.printStackTrace();
			return null;
		}
	}
	...
	public void updateName(Player player) {
		Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
			...
			try {
				Connection conn = getConnection();
				ResultSet r = conn.createStatement()
						.executeQuery("select id from " + tablePrefix + "players where id='" + uuid.toString() + "'");

				if (r.next()) {
					conn.createStatement().executeUpdate("update " + tablePrefix + "players set name='"
							+ player.getName() + "' where id='" + uuid.toString() + "'");
				} else {
					conn.createStatement().executeUpdate("insert into " + tablePrefix + "players" +
							"(id, material, name, gamesplayed) values" +
				...
				Bukkit.getLogger().severe("[NaturalParkour] An error occurred while trying to update name for player "
						+ player.getName() + ":");
				e.printStackTrace();
			}
		});
	...
	public TopEntry getTopPosition(int position, String area) {
		...
		try {
			...
			ResultSet r = conn.createStatement().executeQuery(
					"select * from " + tablePrefix + "scores where area='" + area + "' order by score desc limit "
							+ (position - 1) + "," + position);
			if (!r.next()) {
				closeConn(conn, r);
	...
	public int getGamesPlayed(UUID uuid) {
		try {
			Connection conn = getConnection();
			ResultSet r = conn.createStatement().executeQuery(
					"select gamesplayed from " + tablePrefix + "players where id='" + uuid.toString() + "'");
			if (!r.next()) {
				closeConn(conn, r);
				return 0;
			}
			...
			Bukkit.getLogger().severe("[NaturalParkour] An error occurred when attempting to read from database:");
			e.printStackTrace();
			return -1;
		}
	}
	...
	public void addToGamesPlayed(UUID uuid) {
		...
		try {
			...
			if (r.next()) {
				conn.createStatement().executeUpdate("update " + tablePrefix + "players set gamesplayed='" + newGP
						+ "' where id='" + uuid.toString() + "'");
			}
			closeConn(conn, r);
		} catch (SQLException e) {
			Bukkit.getLogger().severe(
					"[NaturalParkour] An error occurred while trying to update gamesplayed for uuid " + uuid + ":");
			e.printStackTrace();
		}
	}
