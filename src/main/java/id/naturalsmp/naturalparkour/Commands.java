...
import id.naturalsmp.naturalparkour.game.Difficulty;
...
public class Commands implements CommandExecutor {
	...
	public Commands(Main plugin) {
		pl = plugin;
		this.man = pl.man;
		pl.getCommand("naturalparkour").setExecutor(this);
		msgs = pl.msgs;
		scores = pl.scores;
		...
		switch (args[0].toLowerCase()) {
			case "start":
				...
				if (pl.man.getAreas().size() == 0) {
					...
					if (sender.hasPermission("naturalparkour.setup")) {
				...
				if (sply != null) {
					if (Arrays.asList(config.getString("start-disabled-worlds").split(","))
							.contains(sply.getWorld().getName())) {
				...
				if (args.length <= 1) {
					...
					if (pt != null) {
						if (!sender.hasPermission("naturalparkour.start.others")) {
						...
						if (!sender.hasPermission("naturalparkour.start.area")) {
							sender.sendMessage(msgs.get("noperm", sply));
							return true;
						}
					...
					if (!(sender.hasPermission("naturalparkour.start.others")
							&& sender.hasPermission("naturalparkour.start.area"))
							&&
							!sender.hasPermission("naturalparkour.start.othersarea")) {
						sender.sendMessage(msgs.get("noperm", sply));
						return true;
					}
				...
				return true;
			case "cachedebug":
				if (!sender.hasPermission("naturalparkour.debug")) {
					sender.sendMessage(msgs.get("noperm", sply));
					return true;
				}
			...
			case "blocks":
				if (args.length > 1) {
					if (!sender.hasPermission("naturalparkour.selector.openothers")) {
						sender.sendMessage(msgs.get("noperm", sply));
						return true;
					}
					...
					if (!sender.hasPermission("naturalparkour.selector")) {
						sender.sendMessage(msgs.get("noperm", sply));
						return true;
					}
					if (sply != null) {
			...
			case "reset":
				...
				if (args.length > 1) {
					...
					sender.sendMessage(msgs.get("reset.success").replaceAll("\\{PLAYER}",
							p.getName() == null ? p.getUniqueId().toString() : p.getName()));
				} else {
			...
			case "top":
				...
				StringBuilder top = new StringBuilder((area == null ? msgs.get("top.header", sply)
			...
			case "edit":
				...
				if (!sply.hasPermission("naturalparkour.setup")) {
					sender.sendMessage(msgs.get("noperm", sply));
					return true;
				}
				...
				if (args.length > 1) {
					switch (args[1].toLowerCase()) {
						...
						case "save":
							...
							if (n(editing.get("pos1")) || n(editing.get("pos2")) || n(editing.get("diff"))
									|| n(editing.get("name"))) {
			...
			case "difficulties":
				...
				return true;
			case "reload":
				if (!sender.hasPermission("naturalparkour.reload")) {
				...
				if (args.length >= 2) {
					...
					sender.sendMessage(msgs.get("reload.usage").replaceAll("\\{CMD}", label).replaceAll("\\{POSS}",
							poss.toString()));
				}
	...
	public String getMainHelp(Player p, String cmd) {
		...
		if (checkPerm("naturalparkour.start.area", p))
			c.add(msgs.get("commands.help.startarea", p));
		if (checkPerm("naturalparkour.start.others", p))
			c.add(msgs.get("commands.help.startothers", p));
		c.add(msgs.get("commands.help.list", p));
		c.add(msgs.get("commands.help.top", p));
		if (checkPerm("naturalparkour.setup", p))
			c.add(msgs.get("commands.help.areas", p));
		if (checkPerm("naturalparkour.setup", p))
			c.add(msgs.get("commands.help.setup", p));
		if (checkPerm("naturalparkour.portals", p) || checkPerm("naturalparkour.setup", p))
			c.add(msgs.get("commands.help.portals", p));
