...
import id.naturalsmp.naturalparkour.utils.MaterialParser;
...
public class PkJump {
	...
	/**
	 * Creating a jump for the player. Will calculate best possible direction to
	 * place the block.
	 * 
	 * @param ply  A {@link id.naturalsmp.naturalparkour.game.PkPlayer PkPlayer}
	 ...
	 * 
	 * @param block a {@link org.bukkit.Location Location} of the block to get the
	 *              score of
	 * @param from  a {@link org.bukkit.Location Location} of the 'from' position of
	 *              the previous jump
	...
	public static int getBlockScore(Location block, Location from, PkArea area, PkPlayer ply, float yaw) {
		...
		for (int c = 1; c < dirs.length; c++) {
			...
			// ply.getPlayer().sendMessage(dirs[c]+": "+cdistance + ply.msgs.color(" &7 |
			// ")+dirs[c]+" - "+yaw);
		...
		closest = dirs[idx];
		// ply.getPlayer().sendMessage(ply.msgs.color("&eClosest: " +
		// closest+"\n&9-----------------"));
		...
		if (pos(zc) && zero(xc)) {
			...
			Bukkit.getLogger().warning("[NaturalParkour] Could not find direction for jump score!");
		}
	 ...
	 * Get 'from' location
	 * 
	 * @return a {@link org.bukkit.Location Location} that the player is supposed to
	 *         jump from this jump to the next one
	 */
	public Location getFrom() {
		Location r = blocks.get(blocks.size() - 1);
		if (r == null)
			Bukkit.getLogger().warning("[NaturalParkour] Warning: getFrom() returned null!");
		return r;
	}
	 ...
	 * 
	 * @return a {@link org.bukkit.Location Location} that the player is supposed to
	 *         jump to from the previous jump
	 */
	...
	public void place() {
		for (Location l : blocks) {
			...
			l.getBlock().setMetadata("npk-prevtype", new FixedMetadataValue(main, prev));
