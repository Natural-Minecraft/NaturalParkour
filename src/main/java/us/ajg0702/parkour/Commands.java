		...
		switch (args[0].toLowerCase()) {
			case "start":
				...
				if (args.length <= 1) {
					...
					if (!(sender.hasPermission("ajparkour.start.others")
							&& sender.hasPermission("ajparkour.start.area"))
							&&
			...
			case "reset":
				...
				return true;
			case "version":
				sender.sendMessage(msgs.color("&aajParkour &2v&a" + pl.getDescription().getVersion()
						+ " &2by &6ajgeiss0702 &7(https://www.spigotmc.org/members/ajgeiss0702.49935?)"));
				return true;
			case "top":
				String area = null;
				if (args.length > 1) {
